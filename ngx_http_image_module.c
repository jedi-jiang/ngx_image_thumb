 ...
 * image_use_imagick on/off 是否使用ImageMagick库（否则使用GD库）
...
typedef struct
{
  ...
  ngx_flag_t use_imagick;//是否使用ImageMagick库（否则使用GD库）
  char * url;//请求URL地址
  ...
  char buffer[10][255];
  ExceptionInfo * imk_exc_info;//ImageMagick ExceptionInfo 
  image_ptr_t src_im;//原始图片对象
  image_ptr_t dst_im;//目标图片对象
  image_ptr_t w_im;//补白边图片对象
...
static void clean_image_data(void *data);//清除图片Blob数据
static void make_thumb(void *conf);//创建图片对象缩略图,缩略图在此函数中已经处理好,但没有写入到文件
static void water_mark(void *conf);//给图片打上水印
static void thumb_to_string(void *conf);//图片对象数据转换为二进制字符串
...
static void water_image_from(void * conf);//创建水印图片对象
static void image_from(void * conf);//创建原图库对象
...
static ngx_command_t  ngx_http_image_commands[] =
{
  ...
  {
    ngx_string("image_use_imagick"),
    NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
    ngx_conf_set_flag_slot,
    NGX_HTTP_LOC_CONF_OFFSET,
    offsetof(ngx_image_conf_t, use_imagick),
    NULL
  },
  {
...
ngx_log(const char *format, ...)
{
  ...
  static ngx_int_t
ngx_init_process(ngx_cycle_t *cycle)
{
  ...
  static void
ngx_exit_process(ngx_cycle_t *cycle)
{
  MagickCoreTerminus();
}
...
ngx_http_image_create_loc_conf(ngx_conf_t *cf)
{
  ...
  conf->use_imagick = NGX_CONF_UNSET;
...
ngx_http_image_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
  ...
  ngx_conf_merge_value(conf->use_imagick, prev->use_imagick, 0);
  ...
  ngx_conf_merge_value(conf->water_repeat_height,prev->water_repeat_height,800);
  if (conf->use_imagick && conf->water_type != 0)
  {
    PRINT_LOG(NGX_LOG_ERR, cf->log, "TEXT MARK not supported in ImageMagick mode\n");
...
static ngx_int_t ngx_http_image_handler(ngx_http_request_t *r)
{
  ...
  if(file_exists((char*) path.data) == -1)
  {
    ...
    conf->imk_exc_info = AcquireExceptionInfo();
    ...
    if( conf->dest_type > 0 )
    {
      if (parse_image_info(conf) == 0)//解析并处理请求的图片URL
      {
        if (conf->width == 0 || conf->height == 0) {
          conf->width = conf->src_width;
          conf->height = conf->src_height;
        }
        make_thumb(conf);//生成图片缩略图
        water_mark(conf);//图片打上水印
        thumb_to_string(conf);//图片对象转换成二进制字符串
        if(conf->image_output == 0)
        {
          write_img(conf);//保存图片缩略图到文件
        }
        DestroyExceptionInfo(conf->imk_exc_info);
...
static void thumb_to_string(void *conf)
{
  ...
  if (info->use_imagick)
  {
    ImageInfo *tmp_ii = CloneImageInfo((ImageInfo*) NULL);
    switch(info->dest_type)
    {
      case NGX_IMAGE_PNG:
        strcpy(info->dst_im.imk_image->magick, "png");
        info->img_data = ImageToBlob(tmp_ii, info->dst_im.imk_image, (size_t *)(&info->img_size), info->imk_exc_info);
        break;
      case NGX_IMAGE_JPEG:
        strcpy(info->dst_im.imk_image->magick, "jpeg");
        info->dst_im.imk_image->quality = info->jpeg_quality;
        info->img_data = ImageToBlob(tmp_ii, info->dst_im.imk_image, (size_t *)(&info->img_size), info->imk_exc_info);
        break;
      case NGX_IMAGE_GIF:
        strcpy(info->dst_im.imk_image->magick, "gif");
        info->img_data = ImageToBlob(tmp_ii, info->dst_im.imk_image, (size_t *)(&info->img_size), info->imk_exc_info);
        break;
    }
    DestroyImageInfo(tmp_ii);
  }
  else
  {
    switch(info->dest_type)
    {
      case NGX_IMAGE_PNG:
        info->img_data = gdImagePngPtr(info->dst_im.gd_image,&info->img_size);
        break;
      case NGX_IMAGE_GIF:
        info->img_data = gdImageGifPtr(info->dst_im.gd_image,&info->img_size);
        break;
      case NGX_IMAGE_JPEG:
        info->img_data = gdImageJpegPtr(info->dst_im.gd_image,&info->img_size,info->jpeg_quality);
        break;
    }
  }
  ...
  if (info->dst_im.image) 
  {
    if (info->use_imagick)
      DestroyImage(info->dst_im.imk_image);
    else
      gdImageDestroy(info->dst_im.gd_image);
    info->dst_im.image = NULL;
  }
}

static void clean_image_data(void *data){
  ngx_image_conf_t *info = data;
  if (info->img_data) 
  {
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, 
        "FUNC  clean_image_data, info->img_data = %p, info->img_size = %d \n", 
        info->img_data, info->img_size);
    if (info->use_imagick)
      RelinquishMagickMemory(info->img_data);
    else
      gdFree(info->img_data);
    info->img_data = NULL;
  }

  if (info->src_im.image) 
  {
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, 
        "FUNC  gd_clean_data, destroy src_im, info->src_im = %p \n", info->src_im.image);
    if (info->use_imagick)
      DestroyImage(info->src_im.imk_image);
    else
      gdImageDestroy(info->src_im.gd_image);
    info->src_im.image = NULL;
  }

  if (info->dst_im.image) 
  {
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, 
        "FUNC  gd_clean_data, dstroy dst_im, info->dst_im = %p \n", info->dst_im.image);
    if (info->use_imagick)
      DestroyImage(info->dst_im.imk_image);
    else
      gdImageDestroy(info->dst_im.gd_image);
  ...
  {
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, 
        "FUNC  gd_clean_data, destroy w_im, info->w_im = %p \n", info->w_im.image);
    if (info->use_imagick)
      DestroyImage(info->w_im.imk_image);
    else
      gdImageDestroy(info->w_im.gd_image);
    info->w_im.image = NULL;
  }

  if (info->water_im.image) 
  {
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, 
        "FUNC  gd_clean_data, destroy water_im, info->water_im = %p \n", info->water_im.image);
    if (info->use_imagick)
      DestroyImage(info->water_im.imk_image);
    else
      gdImageDestroy(info->water_im.gd_image);
    info->water_im.image = NULL;
  }
}
...
static void make_thumb(void *conf)
{
  ...
  if (info->use_imagick)
  {
    ...
    QueryMagickColor("white", &background, info->imk_exc_info);
    info->dst_im.imk_image = NewMagickImage(tmp_ii, info->width, info->height, &background);
    DestroyImageInfo(tmp_ii);
    float x_scale = (float) info->width / info->src_im.imk_image->columns;
    float y_scale = (float) info->height / info->src_im.imk_image->rows;
    float scale = x_scale < y_scale ? x_scale : y_scale;
    int scaled_w = (int) (info->src_im.imk_image->columns * scale);
    int scaled_h = (int) (info->src_im.imk_image->rows * scale);
    Image * scaled = ScaleImage(info->src_im.imk_image, scaled_w, scaled_h, info->imk_exc_info);
    if (info->imk_exc_info->severity != UndefinedException)
      CatchException(info->imk_exc_info);
    int scaled_x = (info->width - scaled_w) / 2;
    int scaled_y = (info->height - scaled_h) / 2;
    CompositeImage(info->dst_im.imk_image, AtopCompositeOp, scaled, scaled_x, scaled_y);
    DestroyImage(scaled);
  }
  else
  {
    int colors = 0;
    int transparent = -1;
    info->dst_im.gd_image = gdImageCreateTrueColor(info->width,info->height);    //这里根据转换后的参数生产图片
    colors = gdImageColorsTotal(info->src_im.gd_image);
    transparent = gdImageGetTransparent(info->src_im.gd_image);
    PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, "FUNC  make_thumb \n"); 
    if (transparent == -1)
    {
      gdImageSaveAlpha(info->src_im.gd_image, 1);
      gdImageColorTransparent(info->src_im.gd_image, -1);
      if(colors == 0)
      {
        gdImageAlphaBlending(info->dst_im.gd_image, 0);
        gdImageSaveAlpha(info->dst_im.gd_image, 1);
      }
      if(colors)
      {
        gdImageTrueColorToPalette(info->dst_im.gd_image, 1, 256);
      }
    }
    if(info->w_margin == 1)
    {

      info->w_im.gd_image = gdImageCreateTrueColor(info->width,info->height);
      gdImageFilledRectangle(info->w_im.gd_image, 0, 0, info->width,info->height, gdImageColorAllocate(info->w_im.gd_image, 255, 255, 255));
      if (info->dst_im.gd_image) 
      {
        gdImageDestroy(info->dst_im.gd_image);
        info->dst_im.gd_image = NULL;
      }
      info->dst_im.gd_image = gdImageCreateTrueColor(info->max_width,info->max_height);
      gdImageFilledRectangle(info->dst_im.gd_image, 0, 0, info->max_width,info->max_height, gdImageColorAllocate(info->dst_im.gd_image, 255, 255, 255));
      gdImageCopyResampled(info->w_im.gd_image, info->src_im.gd_image, 0, 0, info->src_x, info->src_y,info->width, info->height, info->src_w,info->src_h);
      gdImageCopyResampled(info->dst_im.gd_image, info->w_im.gd_image, info->dst_x,info->dst_y, 0, 0,info->width, info->height, info->width, info->height);
      gdImageDestroy(info->w_im.gd_image);
      info->w_im.gd_image = NULL;
    }
    else
    {
      gdImageCopyResampled(info->dst_im.gd_image,info->src_im.gd_image,info->dst_x,info->dst_y,info->src_x,info->src_y,info->width,info->height,info->src_w,info->src_h);
    }
  }
  ...
  if(info->water_status)//如果水印功能打开了
  {
    ...
    if(info->water_type == 0)//如果为图片水印
    {
      if(file_exists((char *)info->water_image.data) == 0)//判断水印图片是否存在
      {
        ...
        if(info->water_im.image == NULL)//判断对象是否为空
        {
          return;//水印文件异常
        } else {
          if (info->use_imagick)
          {
            water_w = info->water_im.imk_image->columns;
            water_h = info->water_im.imk_image->rows;
          }
          else
          {
            water_w = info->water_im.gd_image->sx;
            water_h = info->water_im.gd_image->sy;
          }
        }
      }
      else
    ...
    else//文字水印
    {
      if (info->use_imagick)
      {
        // TODO TEXT MARK use ImageMagick
      }
      else
      {
        ...
        if(file_exists((char *)water_font) == 0)//如果水印字体存在
        {
          ...
          int brect[8];
          gdImagePtr font_im;
          font_im = gdImageCreateTrueColor(info->dst_im.gd_image->sx,info->dst_im.gd_image->sy);
          ...
          sscanf(G_str,"%x",&G);
          sscanf(B_str,"%x",&B);
          water_color = gdImageColorAllocate(info->dst_im.gd_image,R,G,B);
    ...
    height = info->height > repeat_h ? repeat_h : info->height;
    for(;;)
    {
      ...
      if(info->water_type == 0)
      {
        if (info->use_imagick)
        {
          CompositeImage(info->dst_im.imk_image, OverCompositeOp, info->water_im.imk_image, posX, offsetY+posY);
        }
        else
        {
          gdImagePtr tmp_im;
          ...
          gdImageCopy(tmp_im, info->dst_im.gd_image, 0, 0, posX, offsetY+posY, water_w, water_h);
          gdImageCopy(tmp_im, info->water_im.gd_image, 0, 0, 0, 0, water_w, water_h);
          gdImageCopyMerge(info->dst_im.gd_image, tmp_im,posX, offsetY+posY, 0, 0, water_w,water_h,info->water_transparent);
      ...
      else
      {
        ...
        if (info->use_imagick)
        {
          PRINT_LOG(NGX_LOG_ERR, info->request->connection->log, "FUNC water_mark: text mark not implemented in ImageMagick mode");
        }
        else
        {
          gdImageAlphaBlending(info->dst_im.gd_image,-1);
          gdImageSaveAlpha(info->dst_im.gd_image,0);
          gdImageStringFT(info->dst_im.gd_image,0,water_color,water_font,info->water_font_size, 0.0, posX, offsetY+posY,water_text);
        }
      }
      offsetY += repeat_h;
...
static int calc_image_info(void *conf)
{
  ...
  if( info->src_type > 0)
  {
    ...
    info->dst_im.image = NULL;
    info->w_im.image = NULL;
    image_from(conf);//读取原图图片到图片对象
    if(info->src_im.image != NULL)
    {

      info->src_width = info->use_imagick ? (int)(info->src_im.imk_image->columns) : info->src_im.gd_image->sx;
      info->src_height = info->use_imagick ? (int)(info->src_im.imk_image->rows) : info->src_im.gd_image->sy;
      info->src_x = 0;
      info->src_y = 0;
      info->dst_x = 0;
      info->dst_y = 0;
  ...
  if (info->use_imagick)
    ...
    gdImageDestroy(info->src_im.image);
  info->src_im.image = NULL;
  return -1;
}
...
static void water_image_from(void * conf)
{
  ...
  {   
    ...
    {
      ImageInfo *tmp_ii = CloneImageInfo((ImageInfo*) NULL);
      info->water_im.imk_image = BlobToImage(tmp_ii, buffer, size, info->imk_exc_info);
      DestroyImageInfo(tmp_ii);
    }
    else
    {
      ...
      {
        case NGX_IMAGE_GIF:
          info->water_im.gd_image = gdImageCreateFromGifPtr(size,buffer);
          break;
        case NGX_IMAGE_JPEG:
          info->water_im.gd_image = gdImageCreateFromJpegPtr(size,buffer);
          break;
        case NGX_IMAGE_PNG:
          info->water_im.gd_image = gdImageCreateFromPngPtr(size,buffer);
          break;
      }
    }
...
static void image_from(void * conf)
{
  ...
  if((read_img(&info->source_file,&size,&buffer, info->request)) == 0)
  {
    if (info->use_imagick)
    {
      ImageInfo *src_img_info = CloneImageInfo((ImageInfo*) NULL);
      info->src_im.imk_image = BlobToImage(src_img_info, buffer, size, info->imk_exc_info);
      DestroyImageInfo(src_img_info);
    }
    else
    {
      ...
      {
        case NGX_IMAGE_GIF:
          info->src_im.gd_image = gdImageCreateFromGifPtr(size,buffer);
          break;
        case NGX_IMAGE_JPEG:
          info->src_im.gd_image = gdImageCreateFromJpegPtr(size,buffer);
          break;
        case NGX_IMAGE_PNG:
          info->src_im.gd_image = gdImageCreateFromPngPtr(size,buffer);
          break;
      }
    }
